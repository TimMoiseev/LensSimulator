...
void RingStrip::createModel()
{
	...
	for (int i = -n; i < n; ++i) {
		vec4 ringDot = rot * vec4(vec3(d * glm::cos(2.0f * glm::pi<float>() * i / n) , d * glm::sin(2.0f * glm::pi<float>() * i / n), h / 2.0f), 0.0); //quat*dot*conjugate_quat - так нельзя, в glm поворот делается так quat*dot
		vec4 oppositeRingDot = rot * vec4(vec3(d * glm::cos(2.0f * glm::pi<float>() * i / n), d * glm::sin(2.0f * glm::pi<float>() * i / n), -h / 2.0f), 0.0);
